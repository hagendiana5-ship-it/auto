...
function App() {
  ...
  const [lang, setLang] = useState<Language>('de'); // Hier kannst du deine Standardsprache lassen
  ...
  const handleServiceClick = (service: ServiceItem) => {
    navigateTo(AppView.BOOKING);
  };

  return (
    <div className="min-h-screen bg-black text-white font-sans selection:bg-brand-400 selection:text-black">
      
      {/* --- NUR DIESER TEIL WURDE FÜR DEIN VIDEO ANGEPASST --- */}
      <div className="fixed inset-0 z-0 overflow-hidden">
        <div className="absolute inset-0 bg-black/40 z-10" /> {/* Dunkler Filter, damit man dein Menü noch sieht */}
        <video 
          autoPlay 
          muted 
          loop 
      ...
      {/* Alles andere bleibt EXAKT wie in deiner Datei */}
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex justify-between items-center h-20">
              <div className="flex items-center gap-2 cursor-pointer group" onClick={() => navigateTo(AppView.HOME)}>
                ...
                <span className="font-display italic font-extrabold text-2xl tracking-tighter">
          ...
          {currentView === AppView.SERVICES && (
            <div className="py-24 px-4 max-w-7xl mx-auto">
              ...
              <div className="grid md:grid-cols-3 gap-6">
                {services.map(s => (
                  <ServiceCard key={s.id} service={s} onClick={() => handleServiceClick(s)} />
                ))}
              </div>
            </div>
          )}
